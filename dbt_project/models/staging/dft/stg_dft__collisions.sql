...
renamed as (
    select
        ...
        cast(strptime({{ adapter.quote('date') }}, '%d/%m/%Y') as date) as collision_date,
        ...
        cast({{ adapter.quote('number_of_vehicles') }} as int) as number_of_vehicles,
        cast({{ adapter.quote('number_of_casualties') }} as int) as number_of_casualties,
        day_of_week,
